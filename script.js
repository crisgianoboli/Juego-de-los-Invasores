    ...
    this.dibuja = function () {
        ...
        if (this.vive) {
            if (imgAni < 4) {
                ...
                //  (imgFile, xini, yini, wing, himg, xpos, ypos, wrez, hrez)
            } else if(imgAni < 8) {
...
function mensaje(cadena) {
    let lon = (canvas.width - (50 * cadena.length)) / 2;
    ctx.fillStyle = "white";
...
function colisiones() {
    for (let i = 0; i < ovnis_array.length; i++) {
...
function disparaEnemigo() {
    for (let i = ovnis_array.length - 1; i > 0; i--) {
        if (ovnis_array[i] != null) {
            ultimos.push(i);
        }
